...
**Задачи**:
- [ ] Удалить импорт `github.com/jmoiron/sqlx` из `container.go:11`
- [ ] Удалить поле `DB *sqlx.DB` из `Container` struct (строка 25)
- [ ] Удалить код инициализации sqlx (строки 73-82)
- [ ] Обновить метод `Close()` - удалить закрытие sqlx DB (строки 224-228)
- [ ] Запустить `go mod tidy` для очистки зависимостей
